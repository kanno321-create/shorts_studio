...
Changes from baseline:
  ...
  - Monkeypatch target adjusted to the same module accordingly.
  - Otherwise identical to Phase 3 Wave 1 harvested baseline (Plan 41-02,
    D-41-04 + HC-1~7 + HC-12/13/14).

Fixtures in tmp_path mimic output/{slug}/ shape seen in real outputs (e.g.
output/elisa-lam/). HC-3 mocks ffprobe via monkeypatch on subprocess.run in
hc_checks module namespace. HC-12 generates synthetic PNGs via cv2 (skipped
if cv2 unavailable).

HC-8 is expected to return verdict='SKIP' in the run_all_hc_checks test
(Phase 50-03 wiring deferred per RESEARCH line 964).
HC-9 returns SKIP when NLM pipeline artifacts absent.
HC-10 returns SKIP when gate_results/ absent.
"""
...
from scripts.hc_checks import hc_checks
...
def test_run_all_hc_checks_returns_13_results(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
