"""Automated HC-1~7 + HC-12/13/14 checks for the hardcoded critical verification layer.

Phase 5 rewrite of the 1129-line harvested baseline at
``.preserved/harvested/hc_checks_raw/hc_checks.py`` (Tier 3 immutable). Per
CONTEXT D-18, all 13 public signatures from the baseline's ``__all__`` are
preserved byte-identical; only import paths, the HC-8 SKIP stub (Phase 50-03
Wave 3e deferred), and the ``_ffprobe_duration`` subprocess timeout have been
adjusted.

HC-10 (inspector coverage) now consumes ``GATE_INSPECTORS`` from
``scripts.orchestrator`` via a lazy import inside the function body to avoid
a module-load circular dependency between this package and the orchestrator.
...
``HCResult`` with structured evidence so QA logs can be audited after the
fact.

HC-12 (text-screenshot detection) is per-image and called separately from
``run_all_hc_checks``. See D-41-04: OpenCV primary, pytesseract optional
2nd-pass boundary override.
"""
...
# CircuitBreaker-compatible default timeout for ffprobe subprocess calls
# (RESEARCH line 971 Wave 5 Plan 3 — timeout protection for hanging ffprobe
# processes). Overridable per-call for tests.
...
def check_hc_2(output_dir: Path) -> HCResult:
    """Title line1 <= 12 chars, line2 <= 10 chars."""
    script_path = output_dir / "script.json"
    if not script_path.exists():
...
def _ffprobe_duration(path: Path, timeout_s: int = _FFPROBE_TIMEOUT_S) -> float:
    """Run ffprobe with an explicit timeout (CircuitBreaker-compatible).

    Wave 5 Plan 3 (RESEARCH line 971) requires timeout protection for hanging
    ffprobe processes. ``subprocess.TimeoutExpired`` propagates to the caller
    as a ``subprocess.SubprocessError`` so :func:`check_hc_3` converts it to
    a FAIL verdict with diagnostic evidence.
    """
...
# ---------- HC-8 / HC-9 / HC-10 (semi-public; Phase 50-03) ----------


def check_hc_8_diagnostic_five(output_dir: Path) -> HCResult:
    """HC-8 -- Phase 50-03 5-question diagnostic (D-50-16, Plan 50-03 3e).

    Skeleton: production implementation dispatches an LLM judge with the
    fixed 5-question prompt and returns FAIL if 2+ NO.

    Q1: opening 1-second scroll-stop sentence?
    Q2: misunderstanding / conflict / twist in first 10 seconds?
    Q3: 1+ number / comparison / case example?
    Q4: screen imaginable per cut?
    Q5: curious about the next episode at the end?

    Current surface: SKIP (Phase 50-03 Wave 3e wiring deferred per
    RESEARCH line 964 -- out of scope Phase 5, stub preserved).
    """
...
def check_hc_9_pipeline_order(output_dir: Path) -> HCResult:
    ...
    if script_converted.get("nlm_source_hash") != raw_script_hash:
        mismatches.append(
            "script_converted.nlm_source_hash mismatch vs raw_script.md"
        )
    if script.get("converted_hash") != converted_hash:
        mismatches.append(
            "script.converted_hash mismatch vs script_converted.json"
        )
...
def check_hc_10_inspector_coverage(output_dir: Path) -> HCResult:
    ...
    result artifact during Script gate execution. Prevents silent skip of
    Phase 50 Script Gate inspectors (hook-strength / rhythm / fun etc.).

    Skeleton: looks for ``gate_results/`` directory populated by harness
    during production runs. SKIPs if ``gate_results/`` absent (pre-integration
    test).

    Phase 5 note: the lazy import of ``GATE_INSPECTORS`` from
    ``scripts.orchestrator`` (NOT ``scripts.orchestrator.harness``) avoids a
    circular dependency between this package and the orchestrator package at
    module load time. Key lookup tries both uppercase (``"SCRIPT"``, Phase 5
    naming) and lowercase (``"script"``, harvested baseline naming) for
    ...
    # Lazy import -- avoids circular dependency and import-time failure if
    # scripts.orchestrator is not yet loaded (RESEARCH Pitfall 5). Plan 07
    # exposes GATE_INSPECTORS via scripts.orchestrator.__init__ re-export.
    from scripts.orchestrator import GATE_INSPECTORS

    # Phase 5 Plan 07 uses uppercase gate keys (e.g. "SCRIPT"). Fall back to
    # the harvested-baseline lowercase key for safety.
    script_inspectors: list[str] = []
    for key in ("SCRIPT", "script"):
        if key in GATE_INSPECTORS:
            script_inspectors = list(GATE_INSPECTORS[key])
            break

    expected_script = set(script_inspectors)
    executed = {
...
def run_all_hc_checks(output_dir: Path) -> list[HCResult]:
    """Run HC-1 through HC-7 + HC-6.5 + HC-8/9/10 + HC-13/14 and persist results.
