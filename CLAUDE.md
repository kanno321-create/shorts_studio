...
5. `.claude/memory/MEMORY.md` — 로컬 메모리 인덱스 (9개 박제 메모리)
6. `.env` 파일 존재 확인 — **API key 전부 여기에 있음. 대표님께 재질문 절대 금지** (F-CTX-01, `.claude/memory/reference_api_keys_location.md` 참조)

> **강제 로드**: `.claude/hooks/session_start.py` 가 항목 2+5+6 을 매 세션 system context 로 자동 주입. 텍스트 지시가 아니라 코드 강제. F-CTX-01 재발 방지.
