# Stack Research — naberal-shorts-studio

**Domain:** AI 자동화 YouTube Shorts 제작 스튜디오 (주 3~4편 / 한국어 콘텐츠)
**Researched:** 2026-04-18
**Confidence:** HIGH (주요 선택지), MEDIUM (가격·월정산 수치 — 플랜 변동 가능)
**Consumer:** `.planning/roadmap` — Phase 4~7 구현 단계 기술 선택 근거

---

## Summary

**가장 중요한 10개 결정 (한 줄 요약)**
...
**월 예상 비용 (16 videos): $55~180 범위** (섹션 "Cost Estimate" 참조) — 최소 스택 ~$55, 프리미엄 스택 ~$180.

**Producer-Reviewer (D-3) 호환성:** 모든 선택이 이중 생성 패턴 지원. LLM은 Producer=Sonnet / Reviewer=Opus 비대칭 구성 권장.
...
# === Node.js (Remotion) ===
npm install remotion@^4 @remotion/cli@^4 @remotion/bundler@^4 @remotion/renderer@^4
...
**환경 변수 (`.env` — harness STRUCTURE.md Whitelist):**
```bash
ANTHROPIC_API_KEY=...
KLING_ACCESS_KEY=...
KLING_SECRET_KEY=...
...
TYPECAST_API_KEY=...
ELEVENLABS_API_KEY=...
GEMINI_API_KEY=...        # Nano Banana Pro 용
YOUTUBE_OAUTH_CLIENT_JSON=...  # path
...
**가정:** 쇼츠 평균 45초, 스크립트 평균 400자 (한국어), 썸네일 1장/영상, 재시도율 20% (Producer-Reviewer 이중 생성 포함).
...
- **D-6 3중 방어선**:
  - **Hook**: pre_tool_use.py가 `skip_gates=True` 차단
  - **Dispatcher**: 오케스트레이터 state machine이 GATE 통과 없이 다음 단계 진입 물리 차단
  - **Audit**: `harness-audit` 스킬이 각 에이전트 description 1024자 + rubric 통과 확인
...
*Stack research for: AI-automated YouTube Shorts production studio (Korean, 3~4 videos/week, naberal_harness v1.0.1 기반)*
*Researched: 2026-04-18*
*다음 단계: `.planning/roadmap` Phase 4~7 구현 계획에 이 스택을 반영*
