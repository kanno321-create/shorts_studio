...
## Current Milestone: v1.0.2 Production Readiness — Live Smoke + Adapter Remediation

**Goal:** Phase 12 까지 표준화된 32 에이전트 + 하네스를 실 환경 smoke 로 검증하고 pre-existing adapter drift 를 청산하여 "외부 수익 발생" Core Value 전까지의 마지막 기술적 gap 을 닫는다.

**Target features:**
- Phase 13 Live Smoke 재도전 — Phase 11 SC#1 (real Claude CLI producer/supervisor 호출) + SC#2 (YouTube 과금 환경 smoke 업로드) 재실행 + 증거 anchoring
- Phase 14 API Adapter Remediation — phase05/06/07 drift 15 failures (veo_i2v + elevenlabs + shotstack) 청산, adapter contract 재확립
- Production Readiness 통합 검증 — pytest full regression clean + 실 과금 smoke 1회 성공 + 증거 로그 anchor
...
- [ ] **REQ-12**: Production Readiness 실 환경 검증 — (a) Phase 11 SC#1 deferred 해소: real Claude CLI producer/supervisor 호출 1회 성공 + 증거 anchor (Phase 13 **구조적 경로 완성, Tier 2 live run 대표님 승인 대기**), (b) Phase 11 SC#2 deferred 해소: YouTube 과금 환경 smoke 업로드 1회 성공 (privacy=unlisted + cleanup=True) + production_metadata 첨부 + video_id anchor (Phase 13 **구조적 경로 완성, Tier 2 live run 대표님 승인 대기**), (c) pre-existing adapter drift 15 failures 전량 청산: veo_i2v / elevenlabs / shotstack contract 재정의 + pytest full regression clean (Phase 14), (d) Core Value 궤도 진입 전 최종 기술 readiness gate 통과 (Phase 14 verification). **Phase 13 portion (a)(b) completed 2026-04-22 with approved_with_deferred closure**: Two-tier pytest strategy (Tier 1 always-run mock-based + Tier 2 opt-in `--run-live`) 가동, 49 Tier 1 tests GREEN + 5 Tier 2 tests collected, `phase13_live_smoke.py` runner + `phase13_acceptance.py` + `budget_counter.py` + `evidence_extractor.py` + `upload_evidence.py` + `provider_rates.py` + `phase13_preflight.py` 인프라 전부 shipped, pytest.ini `live_smoke` marker + `adapter_contract` marker 병존, `wiki/render/adapter_contracts.md` + Phase 14 baseline 보존. Tier 2 live run 은 대표님 실 과금 승인 후 `pytest --run-live` 또는 `python scripts/smoke/phase13_live_smoke.py --live` 수동 실행 (예상 $1.50~$3.00, $5 HARD cap). **Phase 14 portion (c) completed 2026-04-21 with complete_with_deferred closure**: 15→0 failures confirmed (SC#1 phase05/06/07 = 742 passed, Wave 1 sweep + acceptance.py 2회 증명), 30 adapter contract tests + 7 structural validator green, `pytest -m adapter_contract` isolated gate 가동 (ADAPT-06), wiki/render/adapter_contracts.md 7-adapter matrix anchored (ADAPT-05), warn-only Hook 확장. SC#5 full `pytest tests/` 3600s budget 초과 + [86%] 구간 2 failures (Phase 14 scope 밖 — phase08~13 범위) 관찰되어 Phase 11 complete_with_deferred 선례 적용 deferred 처리; 별도 investigation phase 예정. Phase 13 portion (a)(b) 은 대표님 실 환경 smoke 승인 대기.
...
---
*Last updated: 2026-04-22 after Phase 13 (Live Smoke 재도전) completion with approved_with_deferred closure. Phase 13 구조적 경로 완성 (Tier 1 49 tests green + Tier 2 5 tests collected + 6 runner/validator modules shipped). Tier 2 실 과금 live run 은 대표님 승인 후 수동 실행. v1.0.2 milestone 2/2 phases complete (Phase 13 + Phase 14), milestone closure 준비 완료 (`/gsd:complete-milestone`) — Tier 2 live run 이후 최종 closure 권장.*
