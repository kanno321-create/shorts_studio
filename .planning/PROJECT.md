...
- [ ] **REQ-12**: Production Readiness 실 환경 검증 — (a) Phase 11 SC#1 deferred 해소: real Claude CLI producer/supervisor 호출 1회 성공 + 증거 anchor (Phase 13), (b) Phase 11 SC#2 deferred 해소: YouTube 과금 환경 smoke 업로드 1회 성공 (privacy=unlisted + cleanup=True) + production_metadata 첨부 + video_id anchor (Phase 13), (c) pre-existing adapter drift 15 failures 전량 청산: veo_i2v / elevenlabs / shotstack contract 재정의 + pytest full regression clean (Phase 14), (d) Core Value 궤도 진입 전 최종 기술 readiness gate 통과 (Phase 14 verification). **Phase 14 portion (c) completed 2026-04-21 with complete_with_deferred closure**: 15→0 failures confirmed (SC#1 phase05/06/07 = 742 passed, Wave 1 sweep + acceptance.py 2회 증명), 30 adapter contract tests + 7 structural validator green, `pytest -m adapter_contract` isolated gate 가동 (ADAPT-06), wiki/render/adapter_contracts.md 7-adapter matrix anchored (ADAPT-05), warn-only Hook 확장. SC#5 full `pytest tests/` 3600s budget 초과 + [86%] 구간 2 failures (Phase 14 scope 밖 — phase08~13 범위) 관찰되어 Phase 11 complete_with_deferred 선례 적용 deferred 처리; 별도 investigation phase 예정. Phase 13 portion (a)(b) 은 대표님 실 환경 smoke 승인 대기.
...
---
*Last updated: 2026-04-21 after Phase 14 (API Adapter Remediation) completion with complete_with_deferred closure (SC#5 only). Phase 14 core scope (ADAPT-01~06, 15→0 failures, 30 contract tests, 7 adapter doc) validated. SC#5 full regression deferred per Phase 11 precedent. Phase 13 (Live Smoke) remains for v1.0.2 milestone closure.*
