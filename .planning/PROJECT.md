...
- [ ] **REQ-01**: `naberal_harness v1.0.1` Layer 1 상속 + **3-Tier 위키 구조** 구축 (Tier 1: `naberal_harness/wiki/`, Tier 2: `studios/shorts/wiki/`, Tier 3: `studios/shorts/.preserved/harvested/`)
...
- [ ] **REQ-09**: **KPI-driven feedback loop** (YouTube Analytics → `wiki/shorts/kpi_log.md` → 다음 달 Producer 입력으로 반영되는 학습 회로)
- [ ] **REQ-10**: 건강 감사 점수 **≥ 80** + A급 drift **0건** + SKILL 500줄 초과 **0건** 달성 (harness-audit 스킬로 검증)
...
|----------|-----------|---------|
| **D-1**: 3-Tier 위키 채택 (harness / studio / preserved) | 프로젝트별 위키 고립 해결, 지식 재발견 반복 방지 | — Pending |
| **D-2**: FAILURES.md 저수지 패턴 (피드백 즉시 SKILL 수정 금지, 월 1회 batch 리뷰) | 학습 충돌 방지, 대표님 기억 의존 제거 | — Pending |
...
| **D-6**: 하네스 3중 방어선 (Hook 차단 / Dispatcher 강제 / Audit 감사) | 지식 누적 시 에이전트 스킵 구조적 대응 | — Pending |
| **D-7**: state machine 오케스트레이터 (텍스트 체크리스트 완전 폐기) | 에이전트 자의적 순서 조정 물리 차단 | — Pending |
| **D-8**: Harvest 전략 — "Harvest, not Fork" (작동 자산 승계 + drift 덩어리 폐기) | 3~6개월 재발견 회피, shorts_naberal 자산 존중 | — Pending |
...
---
*Last updated: 2026-04-19 after initialization (session #10, Deep Questioning 7-question round approved)*
